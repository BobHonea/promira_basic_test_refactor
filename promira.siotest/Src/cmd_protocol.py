...
class spiTransaction:
  ...
  def setInitialPhase(self):
    self.m_spi_phases=[]
    for phase in [  self.m_descriptor.cmd,self.m_descriptor.address, self.m_descriptor.dummy, self.m_descriptor.data]:
      if phase!=None:
        self.m_spi_phases.append(phase)      
  
    if len(self.m_spi_phases)==0:
      fatalError("null command phase specification")
  ...
  def ioModeContinues(self):
    ...
    if next_phase_index<self.m_phase_count:
      ...
      if nextphase.mode == self.m_spi_phase.mode:
          return True
      elif nextphase.mode == promiraTestApp.SPIIO_DUMMY:
          self.m_dummy_mode=self.m_spi_phase.mode
          return True
      elif self.m_spi_phase.mode == promiraTestApp.SPIIO_DUMMY:
  ...
  '''
  def precedent_cmdspec(self, spi_cmd):
    ...
    for index in cmdspec_indices:
      for cmdspec in self.PHASE_SPECS[index]:
    ...
    for iomode in self.m_spiio_mode_precedence:
      for cmdspec in cmdspecs:
        if cmdspec.max_iowidth==iomode:
          return cmdspec
        
    fatalError("io mode forbidden")
